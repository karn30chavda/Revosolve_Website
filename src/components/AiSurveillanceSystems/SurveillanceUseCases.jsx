...
const useCasesList = [
  ...
  [CurrencyCircleDollar, "Banking & ATM Networks"],
  [Factory, "Manufacturing Plants"],
  [AirplaneTilt, "Airports"],
  [Train, "Metro Rail"],
  [Anchor, "Ports"],
  [Package, "Warehouses"],
  [GearSix, "Industrial Facilities"],
  [Bank, "Government Buildings"],
  [ShieldStar, "Defense Installations"],
  [Heartbeat, "Hospitals"],
  [OfficeChair, "Corporate Campuses"]
];

const SurveillanceUseCases = () => {
  return (
    <section
      id="use-cases"
    ...
    >
      ...
      <div className="max-w-[1240px] mx-auto relative z-10 flex flex-col items-center w-full">
        ...
        <div className="flex flex-wrap items-center justify-center gap-4 sm:gap-5 w-full relative z-10">
          {useCasesList.map(([itemIcon, title], idx) => {
            const Icon = itemIcon;
            return (
              <Motion.div
                key={title}
...
export default SurveillanceUseCases;
