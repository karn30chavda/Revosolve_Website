...
const Navbar = () => {
  ...
  return (
    ...
    >
                    ...
                    >
                      <div className="inline-flex flex-row p-6 gap-8 rounded-2xl bg-[#EAEAFF] shadow-2xl border border-white/20">
                        <div className="shrink-0 flex items-stretch">
                          <img src={item.dropdown.image} alt={item.dropdown.title} className="w-72 object-cover rounded-lg shadow-inner" />
                        </div>
                        <div className="flex flex-col items-start min-w-48 justify-center">
                          <span className="text-[#8e95c4] text-xs font-semibold mb-6 uppercase tracking-widest">{item.dropdown.title}</span>
                          <div className={`grid ${item.dropdown.gridCols} gap-x-12 gap-y-4 w-full`}>
                            {item.dropdown.links.map((link, linkIdx) => (
                              ...
                              >
                                <span className="group-hover/link:underline decoration-[#070784] underline-offset-4 transition-all">{link}</span>
                                <img src="/navbar/dropdown_link_icon.svg" className="w-3.5 h-3.5 opacity-0 -translate-x-2 group-hover/link:opacity-100 group-hover/link:translate-x-0 transition-all duration-300" alt="" />
                              </Link>
                            ))}
                          </div>
                        </div>
      ...
      {isMenuOpen && (
        <div 
          className="min-[1290px]:hidden w-full bg-[#010319] flex flex-col items-start justify-start px-5.5 pb-8 pt-2 gap-0 relative animate-[fadeIn_0.3s_ease-out]"
        >
          <div className="flex flex-col w-full">
            {[
              ...
              { label: 'Company', dropdown: { links: ['About Us', 'Careers', 'Blog'] } }
            ].map((item, idx, arr) => {
