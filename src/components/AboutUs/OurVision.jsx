...
const RotatingAnchor = () => {
  ...
  return (
    <span className="relative inline-flex items-baseline justify-center overflow-visible ml-3">
      <AnimatePresence mode="wait">
        <Motion.span
          ...
          className="font-black font-sans bg-linear-to-r from-[#877BF1] to-[#FCCA71] bg-clip-text text-transparent whitespace-nowrap"
        >
          {VISION_WORDS[i]}
        </Motion.span>
      </AnimatePresence>
...
export const OurVision = () => {
  ...
  return (
    <section
      ref={ref}
      className="relative w-full py-28 md:py-36 font-sans bg-[#01031c]"
    ...
    >
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        <Motion.div
          style={{ scale, opacity }}
          className="w-175 h-175 md:w-225 md:h-225 rounded-full"
          animate={{
            background: [
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        {[0, 1, 2].map((i) => (
          <Motion.div
            key={i}
            className="absolute rounded-full border border-[rgba(252,202,113,0.2)] pointer-events-none"
      ...
      <div className="relative w-[85%] max-w-6xl mx-auto text-center z-10 flex flex-col items-center gap-8">
        ...
        <h2 className="text-[28px] sm:text-[38px] md:text-[44px] lg:text-[50px] leading-tight tracking-tight text-center font-sans max-w-4xl">
          <div className="font-black text-white">To become the world&apos;s</div>
          <div className="my-1 md:my-2 flex items-baseline justify-center flex-wrap gap-2">
        ...
        {/* Signal From The Future Typewriter Text */}
        <div className="mt-6 max-w-xl w-full text-left">
          <div className="text-xs font-mono tracking-widest text-[#FCCA71] uppercase font-semibold mb-2">
            // SIGNAL FROM THE FUTURE
          </div>
          <Typewriter
            lines={VISION_LINES}
            className="text-[#CACBDB] text-sm sm:text-base md:text-lg font-light leading-relaxed min-h-[3.2em]"
          />
        </div>
